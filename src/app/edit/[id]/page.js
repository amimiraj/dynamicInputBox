...
export default function Page({ params }) {
    ...
    const editSubmit = async (event) => {
        ...
        try {
            ...
            if (response.ok) {
                localStorage.setItem("data", "Successfully Edited!");
                router.replace('/list');
            }
        } catch (error) {
            console.log(error)
            ...
            {/* <form onSubmit={editSubmit}>
                <div className='max-w-xl mx-auto'>
                    ...
                    {fields.map((field, index) => (
                        ...
                        <div key={index} className=' mt-1 h-10 text-center'>
                            <div className='join ml-16'>
                                ...
                                <input type="checkbox" name="status" className="checkbox checkbox-accent w-10 h-10 mr-1" checked={field.status} onChange={(e) => inputValue(index, e)} />
                                <input type="text" required="" name="num" className="input input-bordered input-accent w-[60%] h-10 " value={field.num} onChange={(e) => inputValue(index, e)} />
                                <button className="btn btn-sm btn-outline btn-error w-10 h-10 ml-1" onClick={() => removeField(index)}>
                    ...
                    <div className=' mt-2 text-center ml-3'>
                        <button onClick={toggleSelect} className='btn btn-ghost w-[20%] mr-1 h-10'>
                            {fields.every(field => field.status) ? 'Unselect All' : 'Select All'}
                        </button>
                        <input type="submit" name="create" className="btn btn-accent w-[31%] h-10" value="Submit" />
                    </div>
